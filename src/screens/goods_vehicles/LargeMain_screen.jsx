...
import truck_view from "../../assets/Images/truck_view.png";
import _ from 'lodash';
import axios from 'axios';
import DateRangePicker from "../../components/DateRangePicker";
import LargeVehicleList from "../../components/goods_vehicles/LargeVehicleList"

const GOOGLE_API_KEY = MAP_API_KEY;

const LargeMain_screen = ({ route }) => {
  ...
  const [tripType, setTripType] = useState("One Day Trip");
  ...
  const vehicleType = "trucks";
  const { minimumTon, maximumTon } = route.params;
  ...
  const getTruck = async () => {
    ...
    try {
      ...
      const truck = res.data.availableVehicles;
      if (res.status === 200) {
        const filteredTrucks = truck.filter((item) => {
          const vehicleTon = parseFloat(item.vehicleDetails.ton);

          if (maximumTon === "more") {
            return vehicleTon >= minimumTon;
          } else {
            return vehicleTon >= minimumTon && vehicleTon <= maximumTon;
          }
        });
  ...
  const getAllvendor = async () => {
    try {
      ...
      const filteredVendor = data.filter((e) => e.vehicles.trucks.length > 0);
            ...
            renderItem={() => (
              ...
              >
                <View style={styles.content_container}>
                  <Text style={styles.heading_txt}>Choose a Mover</Text>
                  ...
                  <View style={styles.input_field_container}>
                    ...
                    <TextInput
                      ...
                      onChangeText={setReceiversNumber}
...
export default LargeMain_screen;
